...
namespace ProyectoFinalAPI.Controllers
{
    ...
    {
        private readonly InstitutionRepo _db;

        public InstitutionController(InstitutionRepo db)
        {
            _db = db;
        }
        ...
        public async Task<IActionResult> Post([FromBody] Institution institution)
        {
            try
            {
                var res = await _db.create(institution);
                return Ok(res);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Put([FromBody] Institution institution)
        {
            try
            {
                var res = await _db.update(institution);
                return Ok(res);
            }
            catch (Exception ex)
            {
